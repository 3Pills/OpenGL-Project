...
protected:
	...
	void UpdateBones();
public:
	FBXModel(const char* a_szModelPath, vec3 a_pos = vec3(0), float a_roughness = 0.3f, float a_fresnelScale = 2.0f, 
			 mat4 a_modTransform = mat4(1), vec3 a_scale = vec3(1), quat a_rot = quat::tquat());
	virtual ~FBXModel();
